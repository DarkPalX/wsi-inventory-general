...
use App\Models\Custom\{Item, ItemCategory, ItemType, ItemUnit, Supplier, ReceivingHeader, ReceivingDetail, IssuanceHeader, IssuanceDetail, ParHeader, ParDetail};
use DB;
...
class ItemController extends Controller
{
    ...
    public function index()
    {
        ...
        $qry = "SELECT i.id as iid, i.name as iname, i.created_at, 
        ...
        if(isset($_GET['search']) && strlen($_GET['search']) > 0) {
            $qry .= " AND (i.sku LIKE '%".$_GET['search']."%' OR 
                        i.barcode LIKE '%".$_GET['search']."%' OR 
                        i.name LIKE '%".$_GET['search']."%' 
                    )";
        }
        ...
        $qry .= " GROUP BY i.id, i.name, i.sku, i.created_at, c.name, t.name
    ...
    {    
       ...
       return view('theme.pages.custom.items.create', compact('page', 'categories', 'types', 'units', 'suppliers'));
    }

    public function store(ItemRequest $request)
    {
        
        $sku_exists = Item::where('sku', $request->sku)->exists();
        if ($sku_exists) {
        ...
        if (!empty($request->unit_id)) {
            ...
            if(filter_var($unit, FILTER_VALIDATE_INT) == false){
                $new_unit = ItemUnit::create([
                    'name' => $unit,
                    'slug' => ModelHelper::convert_to_slug(ItemUnit::class, $unit),
                    'description' => $unit,
                ]);
                $unit = $new_unit->id;
            }
    ...
    public function show($id)
    {
        ...
        $par_details = ParDetail::where('item_id', $id)
        ...
        $rts = ReceivingDetail::where('item_id', $id)->orderBy('id','desc')
        ->get();
            
        // $par_details = ParDetail::select('*')->where('item_id', $id)->orderByDesc('updated_at')->distinct()->get();
        // dd($par_details);

       return view('theme.pages.custom.items.show', compact('page', 'item', 'stock_card', 'par_details','rts'));
    }
    ...
    public function edit(Item $item)
    {
       ...
       return view('theme.pages.custom.items.edit', compact('page', 'item', 'categories', 'types', 'units', 'suppliers'));
    }
