...

{
    ...
    //     $basicQuery = collect($basicQuery);        //dd($basicQuery);
    ...
    public function index()
    {
        ...
        if(isset($_GET['is_search']) && $_GET['is_search']==1){
           ...
           // $transactions = DB::select($qry)->paginate(30);
            ...
            $qry = "select * from receiving_headers where section_id=". Auth::user()->section_id . " order by id desc";
        }
        /*
        $take = 1; 
        $perPage = $_GET['per_page'] ?? 1;
    ...
    public function search_item(Request $request)
    {
        ...
        $results = Item::query()
            ...
            ->select('items.id', 'items.sku', 'items.name', 'item_types.name as type_name', 'item_units.name as unit_name') // You can add more fields if needed
            ->get();
    ...
    public function search_po_number(Request $request)
    {
        ...
        $results = PurchaseOrderHeader::where('ref_no', 'like', '%' . $query . '%')->where('status', 'POSTED')->where('total_remaining', '>', 0)->get(['ref_no']);
