...
<head>
	...
	<link rel="stylesheet" href="{{ asset('theme/css/colors.php?color=287ad1') }}">
...
<body class="stretched">
	...
	<script>
		$(document).ready(function() {
			$('.dt-button.buttons-copy.buttons-html5').each(function() {
				$(this).html('<i class="fa fa-copy fa-2x text-secondary" title="Copy to Clipboard"></i>');
				$(this).removeClass('dt-button buttons-copy buttons-html5').addClass('btn btn-transparent');
			});
			
			$('.dt-button.buttons-excel.buttons-html5').each(function() {
				$(this).html('<i class="fa fa-table-cells fa-2x text-success" title="Download as Excel File"></i>');
				$(this).removeClass('dt-button buttons-excel buttons-html5').addClass('btn btn-transparent');
			});
			
			$('.dt-button.buttons-pdf.buttons-html5').each(function() {
				$(this).html('<i class="fa fa-file-pdf fa-2x text-danger" title="Download as PDF File"></i>');
				$(this).removeClass('dt-button buttons-pdf buttons-html5').addClass('btn btn-transparent');
			});
			
			$('.dt-button.buttons-csv.buttons-html5').each(function() {
				$(this).html('<i class="fa fa-file-csv fa-2x text-info" title="Download as CSV File"></i>');
				$(this).removeClass('dt-button buttons-csv buttons-html5').addClass('btn btn-transparent');
			});
			
			$('.dt-button.buttons-print').each(function() {
				$(this).html('<i class="fa fa-print fa-2x text-primary" title="Print"></i>');
				$(this).removeClass('dt-button buttons-print').addClass('btn btn-transparent');
			});
			
			$('.dt-button.buttons-collection.buttons-colvis').each(function() {
				// $(this).html('<button class="btn dropdown-toggle">Column Visibility</button>');
				$(this).html('<i class="fa fa-eye fa-2x text-secondary" title="Column Visibility"></i> <i class="fa fa-chevron-down" style="font-size:10px" title="Column Visibility"></i>');
				$(this).removeClass('dt-button buttons-collection buttons-colvis').addClass('btn btn-transparent');
			});
