										...
										<table class="table table-hover" id="selected_items_table">
											<thead>
												<tr>
													...
													<th width="15%" class="text-end">Accountable</th>
													<th width="5%"></th>
												</tr>
											</thead>
											<tbody>
												@foreach($issuance_details as $issuance_detail)
													<tr>
														...
														<td>
															...
															<input name="item_name[]" type="text" value="{{ $issuance_detail->item()->withTrashed()->first()->name }}" hidden>
														</td>
														<td>
														...
														<td>
															<input name="quantity[]" 
																type="number" step="1" value="{{ $issuance_detail->quantity }}" min="1" max="{{ $issuance_detail->item->inventory }}" onclick="this.select()" class="text-end"
																style="width:100%;"
																oninput="
														...
														<td class="text-end">
															{{-- <input type="text" id="search_receiver_individual" name="individual_receiver[]" value="{{ App\Models\Custom\ParHeader::getParDetail($issuance_detail->issuance_header_id, $issuance_detail->item_id) }}" class="text-end search_receiver_individual" autocomplete="off" placeholder="Search employee" list="search_receiver_list" onkeypress="if(event.key === 'Enter') { event.preventDefault(); }" onclick="select()" required> --}}
															<input type="text" id="search_receiver_individual" name="individual_receiver[]" value="{{ App\Models\Custom\ParHeader::getEmployeeIdName($issuance_detail->issuance_header_id, $issuance_detail->item_id) }}" class="text-end search_receiver_individual" autocomplete="off" placeholder="Search employee" list="search_receiver_list" onkeypress="if(event.key === 'Enter') { event.preventDefault(); }" onclick="select()" required>
									...
									data.results.forEach(item => {
										resultsTableBody.append(`
											<tr>
												...
												<td><button type="button" name="insert_selected[]" class="btn btn-outline-primary add-item-btn" data-id="${item.id}" data-sku="${item.sku}" data-name="${item.name}" data-unit="${item.unit}" data-inventory="${item.inventory}" data-price="${item.price}">Add</button></td>
											</tr>
										`);
									});
								} else {
		...
		document.addEventListener('click', function(event) {
			...
			if (target.classList.contains('add-item-btn')) {
				...
				if (!exists) {
					...
					let receiverCell = newRow.insertCell(8);
					...
					let extraCell = newRow.insertCell(9);
					extraCell.innerHTML = '&nbsp;'
