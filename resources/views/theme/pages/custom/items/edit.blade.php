								...
								<div class="tab-content">
									<div class="tab-pane fade show active" id="main-tab" role="tabpanel" tabindex="0">
										<div class="form-group row">
											...
											<div class="col-sm-10">
												<input type="text" class="form-control {{ $errors->has('sku') ? 'is-invalid' : '' }}" id="sku" name="sku" value="{{ old('sku', $item->sku) }}" required>
												@error('sku')
										...
										<div class="form-group row">
											...
											<div class="col-sm-10">
												<input type="text" class="form-control {{ $errors->has('barcode') ? 'is-invalid' : '' }}" id="barcode" name="barcode" value="{{ old('barcode', $item->barcode) }}" required>
												@error('barcode')
													<small class="text-danger">{{ $message }}</small>
												@enderror
											</div>
										...
										<div class="form-group row">
											<label class="col-sm-2 col-form-label">Unit of Measurement</label>
											<div class="col-sm-10">
												<select id="unit_id" name="unit_id" class="select-tags form-select {{ $errors->has('unit_id') ? 'is-invalid' : '' }}" aria-hidden="true" style="width:100%;" >
													<option value="">-- SELECT TYPE --</option>
													@foreach($units as $unit)
														<option value="{{ $unit->id }}" {{ old('unit_id', $item->unit->id) == $unit->id ? 'selected' : '' }}>{{ $unit->name }}</option>
													@endforeach
												</select>
												@error('unit_id')
													<small class="text-danger">{{ $message }}</small>
												...
												@foreach($types as $type)
													<div class="form-check form-check-inline">
														<input type="radio" id="type_{{ $type->id }}" name="type_id" 
															   value="{{ $type->id }}" 
															   class="form-check-input {{ $errors->has('type_id') ? 'is-invalid' : '' }}"
															   {{ old('type_id', $item->type->id) == $type->id ? 'checked' : '' }}>
