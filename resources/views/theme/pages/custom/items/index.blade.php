                ...
                <form class="d-flex align-items-center" id="searchForm" style="margin-bottom:10px;font-size: 12px !important;">
                    ...
                    <table width="100%" style="margin-bottom: 0px;">
                        ...
                        <tr>
                            ...
                            <td>
                                <select name="type" id="type" class="form-control" style="font-size:12px;">
                                    <option value="">- All -</option>
                                    @php $types = \App\Models\Custom\ItemType::orderBy('name')->get(); @endphp
                            ...
                            <td width="30%"><input name="search" type="search" id="search" class="form-control" placeholder="Search.."  @if(isset($_GET['search'])) value="{{$_GET['search']}}" @endif style="font-size:12px;"></td>
                            <td>
                ...
                <table class="table table-hover" cellspacing="0" width="100%">
                    <thead class="table-secondary">
                        <tr>
                            ...
                            <th style="width: 5%;" class="text-center">Bar Code</th>
                            <th>SKU</th>
                            {{-- <th style="width: 5%;"></th> --}}
                            <th width="20%">Item</th>
                            <th>Category</th>
                        ...
                        @forelse ($items as $item)
                            ...
                            {{-- <tr id="row{{$item_info->id}}" @if($item_info->Inventory < $item_info->minimum_stock) class="table-warning" title="Inventory is below its limit"  data-row-index="0" @else  data-row-index="1" @endif> --}}
                                ...
                                <td class="text-center">
                                    @if($item_info->barcode != null) 
                                        {!! \App\Models\Custom\Item::generateQrCode($item_info->barcode, 50) !!} 
                                    @else 
                                        <strong><small style="font-size:10px;">NO QR <br>CODE <br>AVAILABLE</small></strong>
                                    @endif
                                </td>
                                <td onclick="event.stopPropagation();">{{ $item_info->sku }}<br></td>
                                {{-- <td><img height="54" width="54" src="{{ env('APP_URL') }}/{{$item_info->image_cover}}" onerror="this.src = '{{ env('APP_URL') }}/images/tflp-logo.png';"/></td> --}}
                                <td>{{ $item_info->name }}</td>
                                <td>{{ $item_info->category->name }}</td>
                                <td>{{ $item_info->unit->name }}</td>
                                <td>{{ $item_info->type->name }}</td>
                                <td onclick="event.stopPropagation();"><a href="{{ route('items.stock-card', $item_info->id) }}" @if($item_info->Inventory > 0 ) class="text-dark" @endif><strong>{{ $item_info->Inventory }}</strong></a></td>
                                <td>{{ $item_info->minimum_stock }}</td>
                                <td class="flex justify-center items-center" onclick="event.stopPropagation();">
                                    ...
                                    @else
                                        <a href="{{ route('items.show', $item_info->id) }}" class="btn btn-light text-info"><i class="bi-eye"></i></a>
                                         @if(RolePermission::has_permission(1,auth()->user()->role_id,1))
        ...
        <div class="modal-dialog modal-md" role="document">
            <div class="modal-content bg-transparent border-0 shadow-none" style="width:80%">
                <div class="flip-card text-center shadow" style="height: 584px;">
                    ...
                    <div class="flip-card-back h-100" style="background-image: url('{{ asset('images/company-logo.png') }}'); background-size: 350px; background-repeat: no-repeat; background-position: center;">
