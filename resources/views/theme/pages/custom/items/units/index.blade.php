    ...
    <div class="wrapper p-5">
        ...
        <div class="row mt-4 mb-3">
            ...
            <div class="col-md-6 d-flex align-items-center justify-content-end">
                ...
                {{-- <a href="{{ route('items.units.create') }}" class="btn btn-primary">Create a Type</a> --}}
            </div>
                ...
                <table id="authors_tbl" class="table table-hover" cellspacing="0" width="100%">
                    ...
                    <tbody>
                        @forelse ($units as $unit)
                            <tr id="row{{$unit->id}}" @if($unit->trashed()) class="table-danger" @endif>
                                <td>
                                    <input type="checkbox" class=" @if($unit->trashed()) item-trashed @else select-item @endif" id="cb{{ $unit->id }}" @if($unit->trashed()) disabled @endif>
                                    <label class="custom-control-label" for="cb{{ $unit->id }}"></label>
                                </td>
                                <td><strong>{{ $unit->name }}</strong></td>
                                <td>{{ $unit->description }}</td>
                                <td>{{ Setting::date_for_listing($unit->created_at) }}</td>
                                <td>{{ Setting::date_for_listing($unit->updated_at) }}</td>
                                <td class="flex justify-center items-center">
                                    @if($unit->trashed())
                                        <a href="javascript:void(0)" class="btn btn-transparent text-primary">&nbsp;</a>
                                        {{-- <a href="javascript:void(0)" class="btn btn-light text-primary" onclick="single_restore({{ $unit->id }})"><i class="fa-solid fa-undo-alt"></i></a> --}}
                                    @else
                                        <a data-bs-toggle="modal" data-bs-target="#edit_type{{ $unit->id }}" class="btn btn-light text-warning"><i class="uil-edit-alt"></i></a>
                                        {{-- <a href="{{ route('items.units.edit', $unit->id) }}" class="btn btn-light text-warning"><i class="uil-edit-alt"></i></a> --}}
                                        <a href="javascript:void(0)" class="btn btn-light text-danger" onclick="single_delete({{ $unit->id }})"><i class="uil-trash-alt"></i></a>
                                    @endif
                                </td>
                            </tr>

                            {{-- EDIT TYPE MODAL --}}
                            <div id="edit_type{{ $unit->id }}" class="modal fade text-start" tabindex="-1" role="dialog" aria-labelledby="centerModalLabel" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <form method="post" action="{{ route('items.units.update', $unit->id) }}">
                                                @csrf
                                                @method('put')
    ...
    <div class="modal fade text-start create-form-modal" tabindex="-1" role="dialog" aria-labelledby="centerModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="post" action="{{ route('items.units.store') }}">
                        @csrf
                        <div class="form-group row">
                            <label for="name" class="col-sm-2 col-form-label">Name</label>
                            <div class="col-sm-10">
...
@section('pagejs')
    ...
    <script>
        let listingUrl = "{{ route('items.units.index') }}";
    ...
    <script>
        function single_delete(id){
            ...
            $('.btn-delete').on('click', function() {
                post_form("{{ route('items.units.single-delete') }}",'',id);
            });
        }
        ...
        function multiple_delete() {
            ...
            if (counter < 1) {
                ...
                $('.btn-delete-multiple').on('click', function() {
                    post_form("{{ route('items.units.multiple-delete') }}", '', selected_items);
                });
            }
        }
        
        function single_restore(id){
            post_form("{{ route('items.units.single-restore') }}",'',id);
        }
        ...
        function multiple_restore() {
            ...
            if (counter < 1) {
                ...
                $('.btn-restore-multiple').on('click', function() {
                    post_form("{{ route('items.units.multiple-restore') }}", '', selected_items);
                });
            }
        }
        
        function post_form(url,status,units){
