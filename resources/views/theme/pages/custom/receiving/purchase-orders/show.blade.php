                            ...
                            <div class="form-group row">
                                <div class="col-sm-12">
                                    <p class="form-control-plaintext">
                                        <strong><small class="rounded text-white {{ $purchase_order->status == 'PENDING' ? 'bg-warning' : ($purchase_order->status == 'CANCELLED' ? 'bg-danger' : 'bg-success') }} p-1">{{ $purchase_order->status }}</small></strong>
                                        <small class="text-secondary" {{ $purchase_order->status == 'PENDING' ? 'hidden' : '' }}> | 
                                    ...
                                    <tbody>
                                        @foreach($purchase_order_details as $purchase_order_detail)
                                            <tr>
                                                ...
                                                <td>
                                                    <input name="quantity[]" class="border-0" type="number" value="{{ $purchase_order_detail->quantity }}" readonly>
                                                </td>
                                                <td class="text-end">
                                                    <input class="subtotal text-end border-0" name="subtotal[]" type="text" value="{{ number_format(($purchase_order_detail->item->price ?? 0) * ($purchase_order_detail->quantity ?? 1), 2, '.', ',') }}" readonly>
                                                </td>
                                                <td>&nbsp;</td>
                                            </tr>
                                        @endforeach
                                        ...
                                        {{-- COMPUTATIONS --}}
                                            <div id="computation-row">
                                                <tr style="pointer-events: none;">
                                                    ...
                                                    <td class="text-end"><input type="text" name="net_total" value="{{ number_format($purchase_order->net_total, 2, '.', ',') }}" class="text-end border-0" readonly></td>
                                                    <td>&nbsp;</td>
                                                </tr>
                                                <tr style="pointer-events: auto;" class="table-borderless">
                                                    <td colspan="5"><input name="item_id[]" type="text" value="0" hidden></td>
                                                    <td class="text-end">VAT (%)</td>
                                                    <td class="text-end"><input type="text" name="vat" value="{{ number_format($purchase_order->vat, 2, '.', ',') }}" class="text-end border-0" step="1" min="0" onclick="this.select()" oninput="this.value = this.value < 0 ? 0 : this.value;" ></td>
                                                    <td>&nbsp;</td>
                                                </tr>
                                                <tr style="pointer-events: none;">
                                                    <td colspan="5"><input name="item_id[]" type="text" value="0" hidden></td>
                                                    <td class="text-end">Grand Total</td>
                                                    <td class="text-end"><input type="text" name="grand_total" value="{{ number_format($purchase_order->grand_total, 2, '.', ',') }}" class="text-end border-0 fw-bold" style="font-size:17px;" readonly></td>
                                                    <td>&nbsp;</td>
                                                </tr>
                                            </div>
                                    </tbody>
