                        ...
                        <div class="card-body">
							...
							<form method="post" action="{{ route('receiving.transactions.update', $transaction->id) }}" enctype="multipart/form-data" onsubmit="return checkSelectedItems();">
								...
								<div class="form-group row">
									...
									<div class="col-sm-10">
										<input type="date" class="form-control" id="date_received" name="date_received" value="{{ $transaction->date_received }}" min="{{ $transaction->date_received }}" required>
									</div>
								</div>
										...
										<table class="table table-hover" id="selected_items_table">
											...
											<tbody>
												@foreach($receiving_details as $receiving_detail)
													<tr>
														<td><button type="button" class="btn btn-outline-danger remove-item-btn" data-id="'+id+'" data-sku="'+sku+'" data-name="'+name+'" data-copies="'+copies+'" data-cost="'+cost+'"><i class="bi-trash"></i></button></td>
														...
														<td>{{ optional(optional($receiving_detail->item)->unit)->name ?? 'N/A' }}</td>
														<td><input name="cost[]" type="number" step="1" class="form-control form-control-sm" value="{{ $receiving_detail->cost }}" min="0" oninput="this.value = this.value < 0 ? 0 : this.value" onclick="select()"></td>
														<td><input name="quantity[]" type="number" step="1" class="form-control form-control-sm" value="{{ $receiving_detail->quantity }}" min="1" oninput="this.value = this.value < 1 ? 1 : this.value" onclick="select()"></td>
